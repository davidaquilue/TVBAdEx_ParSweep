...
run_sim = 600000.0  # ms, length of the simulation We run for longer to have a larger margin for computing FC
cut_transient = 10000.0  # ms, length of the discarded initial segment
...
for simnum in tqdm(range(len(Job_proc))):
    ...
    parameters.parameter_monitor['Raw'] = False  # Else, it will be toooo heavy
    parameters.parameter_monitor['Bold'] = True
    parameters.parameter_monitor['parameter_Bold']['period'] = 2000.0
